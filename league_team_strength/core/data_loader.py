...
class FotMobDataLoader:
    ...
    def get_all_matches(self, season_year: Optional[int] = None, 
        ...
        Returns: DataFrame with match_id, league_id, season, teams, scores, dates
        """
        query = """
            SELECT 
                md.match_id,
    ...
    def get_match_stats(self, match_ids: List[int] = None) -> pd.DataFrame:
        ...
        Returns: DataFrame with match_id and all stats
        """
        query = """
            SELECT 
                match_id,
    ...
    def get_player_match_performance(self, player_id: int, 
        ...
        query = """
            SELECT 
                ...
                -- Aggregate match-level stats if needed
                -- This assumes you want per-match data
                -- For per-90 calculations, you'll need minutes
                
                -- Get goals/assists from events
                (SELECT COUNT(*) 
                ...
                0 as assists  -- Would need to parse assist data
                
            FROM [dbo].[match_lineup_players] mlp
            INNER JOIN (
                SELECT match_id, league_id, league_name, 
                       YEAR(match_time_utc) as season_year
                FROM [dbo].[match_details]
            ) md ON mlp.match_id = md.match_id
            
            WHERE mlp.player_id = ?
              AND md.season_year BETWEEN ? AND ?
            ORDER BY md.season_year, mlp.match_id
        """
    ...
    def get_league_teams_for_season(self, league_id: int, season_year: int) -> List[int]:
        ...
        query = """
            ...
            FROM (
                ...
                FROM [dbo].[match_details]
                WHERE league_id = ? AND YEAR(match_time_utc) = ?
                UNION
    ...
    def get_transfers_between_leagues(self, source_league_id: int, 
                                     ...
                                     end_season: int) -> pd.DataFrame:
        """
        Identify players who moved between leagues.
        This requires comparing player appearances across seasons.
        """
        query = """
            ...
            WHERE source.league_id = ? AND dest.league_id = ?
              AND source.appearances >= 10  -- Minimum appearances to count
    ...
    def get_european_matches(self, season_year: int) -> pd.DataFrame:
        ...
        query = f"""
            SELECT 
                md.match_id,
                md.league_id as competition_id,
            ...
            FROM [dbo].[match_details] md
            LEFT JOIN [dbo].[match_stats_summary] mss ON md.match_id = mss.match_id
            WHERE md.league_id IN ({placeholders})
              AND YEAR(md.match_time_utc) = ?
              AND md.finished = 1
        """
